...
| CORR-20260716-0006 | 3835 | B 保險實務-分類 | 04 人身保險意義、功能、分類 | `input/JY-人身保險.pdf` / JY價值筆記 | P.93 | 8 | 3 | 3 | 3 | `option_pollution`, `truncated_question`, `ocr_parse_error` | `fixed` | `applied` | `all_questions.json` id=3835；SQLite `questions.id`=3835（已修正） | 原稿採「句首＋四個主詞選項＋共同句尾」版面；正式題庫已由受控腳本同步修正題幹與選項邊界，答案維持3。此題不是答案錯誤。 |
...
### CORR-20260716-0005 證據與建議修正

- question_id：3840
- source：`input/JY-人身保險.pdf` P.93 Q13
- current_answer：`"2"`
- source_answer：`"4"`
- expected_answer：`"4"`
...
- fix_target：`all_questions.json` id=3835；SQLite `questions.id`=3835（已修正）
- fixed_question_id：3835
- apply_script：`tools/apply_id3835_question_structure_correction.py`
- closeout_file：`docs/answer_audit/id3835_question_structure_correction_apply_closeout_20260716.md`
- notes：原稿共同句尾「出極少的錢……保障他本人或親屬安樂的生活」應屬題幹，不屬於第4選項「保險公司的員工」。已由受控腳本同步修正 JSON 與 SQLite 的題幹及選項邊界；答案3維持不變。
...
ID3835 已完成備份、受控修正及資料一致性驗證；題數不變，差異僅 ID3835，SQLite `integrity_check = ok`。Web 顯示驗證另行記錄。
...
| `MISS-20260716-0008` | 29 | 4141 | 4141 | B 保險實務-分類候選 | 03 保險費架構、解約金、準備金、保單紅利 | `input/JY-人身保險.pdf` / JY價值筆記 | P.90 | 1 | 1 | `source_mapping_error`, `outdated_law` | `included` | `applied` | `all_questions.json` id=4141；SQLite `questions.id`=4141（已新增） | `docs/corrections/p90_q29_jy_missing_candidate_20260716.md` | 經官方法規佐證及版本化改寫核准後，已正式補入為 ID4141。`all_questions.json` 與 SQLite `questions` 均已新增；inclusion_status：`applied`；apply_script：`tools/apply_missing_question_4141_q29.py`；closeout_file：`docs/answer_audit/jy_unit03_q29_4141_apply_closeout_20260716.md`。 |
...
| `MISS-20260716-0010` | 35 | 4139 | 4139 | B 保險實務-分類候選 | 03 保險費架構、解約金、準備金、保單紅利 | `input/JY-人身保險.pdf` / JY價值筆記 | P.91 | 3 | 2 | `source_mapping_error`, `outdated_law` | `included` | `applied` | `all_questions.json` id=4139；SQLite `questions.id`=4139（已新增） | `docs/corrections/p91_q35_jy_missing_candidate_20260716.md` | 經官方法規佐證及版本化改寫核准後，已正式補入為 ID4139。`all_questions.json` 與 SQLite `questions` 均已新增；inclusion_status：`applied`；apply_script：`tools/apply_missing_questions_4139_4140.py`；closeout_file：`docs/answer_audit/missing_questions_4139_4140_apply_closeout_20260716.md`。 |
| `MISS-20260716-0011` | 36 | 4140 | 4140 | B 保險實務-分類候選 | 03 保險費架構、解約金、準備金、保單紅利 | `input/JY-人身保險.pdf` / JY價值筆記 | P.91 | 1 | 3 | `source_mapping_error`, `outdated_law` | `included` | `applied` | `all_questions.json` id=4140；SQLite `questions.id`=4140（已新增） | `docs/corrections/p91_q36_jy_missing_candidate_20260716.md` | 經官方法規佐證及版本化改寫核准後，已正式補入為 ID4140。`all_questions.json` 與 SQLite `questions` 均已新增；inclusion_status：`applied`；apply_script：`tools/apply_missing_questions_4139_4140.py`；closeout_file：`docs/answer_audit/missing_questions_4139_4140_apply_closeout_20260716.md`。 |
...
| `MISS-20260716-0013` | 47 | 4138 | 4138 | B 保險實務-分類候選 | 03 保險費架構、解約金、準備金、保單紅利 | `input/JY-人身保險.pdf` / JY價值筆記 | P.91 | 3 | 3 | `source_mapping_error` | `included` | `applied` | `all_questions.json` id=4138；SQLite `questions.id`=4138（已新增） | `docs/corrections/p91_q47_jy_missing_candidate_20260716.md` | Q47 為正向完整組合題；已正式補入為 ID4138。`all_questions.json` 與 SQLite `questions` 均已新增；inclusion_status：`applied`；apply_script：`tools/apply_missing_question_4138_q47.py`；closeout_file：`docs/answer_audit/jy_unit03_q47_4138_apply_closeout_20260716.md`。 |
...
上述案例中，`MISS-20260716-0004` 已另經等價題審核、ID mapping、備份及受控腳本流程正式補入為 ID4136；`MISS-20260716-0008` 已經官方法規佐證、版本化改寫、ID mapping、備份及受控補入流程正式補入為 ID4141；`MISS-20260716-0013` 亦已完成等價題裁定、ID mapping、備份及受控補入流程，正式補入為 ID4138；其餘案例維持原狀。未補入的漏題候選均不屬於 `ready_to_fix`，不得直接補入正式題庫。
...
| inclusion_status | `applied` |
| new_question_id | 4138 |
| all_questions.json | 已新增 |
| SQLite questions | 已新增 |
| apply_script | `tools/apply_missing_question_4138_q47.py` |
| closeout_file | `docs/answer_audit/jy_unit03_q47_4138_apply_closeout_20260716.md` |

此案例已完成 ID mapping、備份、dry-run、正式補入與資料一致性驗證；原稿證據及等價題裁定仍由既有文件保留追溯。
...
| 29 | P.90 | 4141 | `source_mapping_error`, `outdated_law` | `included` | `MISS-20260716-0008` 經官方法規佐證及版本化改寫核准後，已正式補入為 ID4141。 |
...
| 47 | P.91 | 4138 | `source_mapping_error` | `included` | `MISS-20260716-0013` 已經核准流程正式補入為 ID4138。 |
...
| 29 | P.90 | 1 | 原稿有題且已有截圖確認紀錄；其後完成官方法規佐證與版本化改寫 | `included` | `inclusion_status = applied`；已正式補入為 ID4141，JSON 與 SQLite 已同步新增 |
...
| 47 | P.91 | 3 | 原稿有題、已有截圖確認紀錄，且已裁定可與 ID107 並存 | `included` | `inclusion_status = applied`；已正式補入為 ID4138，JSON 與 SQLite 已同步新增 |
